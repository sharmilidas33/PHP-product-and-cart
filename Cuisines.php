  ...
  if(isset($_POST['add'])){
    ...
    if(isset($_SESSION['cart'])){
        ...
        if(in_array($_POST['product_id'],$item_array_id)){
            ...
            echo "<script>window.location='Cuisines.php'</script>";
        }else{
        ...
        <div class="products" class="row1" style=" margin: 50px 30px;">
            <?php
                ...
                while($row = mysqli_fetch_assoc($result)){
                    if ($row['product_type'] == 'cuisine'){
                    component($row['product_name'],$row['product_price'],$row['product_image'],$row['id'],$row['product_type']);
                    }
                }
            ?>
    </div>
